...
/**
 * ORIGINAL FROM https://github.com/Snoresville/dota2buttemplate_fixed/tree/master
 * REFORMATED BY ThLion_ FOR MY OWN USING
 */
...
loadSettings(CustomNetTables.GetTableValue("settings","updated"));
...
function generateCategory(odata,category_id) {
	...
	if (undefined==data.NAME) {
		category.FindChildTraverse( "SettingCategoryText" ).text = $.Localize("#Option_" + category_id);
		if ($.Localize("#Option_" + category_id + "_Description") != "#Option_" + category_id + "_Description") {
			category.FindChildTraverse( "SettingCategoryText" ).SetPanelEvent( 'onmouseover', function () {
				$.DispatchEvent("DOTAShowTextTooltip", category.FindChildTraverse( "SettingCategoryText" ), $.Localize("#Option_" + category_id + "_Description"));
			} ); 
	...
	if (undefined==data.NAME) {
		category_block.FindChildTraverse( "SettingCategoryBlock_TittleText" ).text = $.Localize("#Option_" + category_id);
	} else {
	...
	if (undefined!==data.TYPE && data.TYPE == "OPTIONAL") {
		...
		setting.FindChildTraverse( "SettingValue" ).text = $.Localize("#Option_ENABLED");
...
function generateSetting_options(data,category_block,category_id,setting_id) {
	...
	if ($.Localize("#Option_" + category_id + "_Option_" + setting_id + "_Description") != "#Option_" + category_id + "_Option_" + setting_id + "_Description") {
		setting.SetPanelEvent( 'onmouseover', function () {
			$.DispatchEvent("DOTAShowTextTooltip", setting, $.Localize("#Option_" + category_id + "_Option_" + setting_id + "_Description"));
		} ); 
	...
	for (let i in data.OPTIONS) {
		...
		if ($.Localize("#Option_" + category_id + "_Option_" + setting_id + "_Option_" + txt) == "#Option_" + category_id + "_Option_" + setting_id + "_Option_" + txt) {
			opt.text = txt;
		} else {
			opt.text = $.Localize("#Option_" + category_id + "_Option_" + setting_id + "_Option_" + txt);
		}
...
function generateSetting_number(data,category_block,category_id,setting_id) {
	...
	if (undefined==data.NAME) {
		setting.FindChildTraverse( "SettingNumberText" ).text = $.Localize("#Option_" + category_id + "_Option_" + setting_id);
	} else {
...
function generateSetting_boolean(data,category_block,category_id,setting_id) {
	...
	if (undefined==data.NAME) {
		setting.FindChildTraverse( "SettingValue" ).text = $.Localize("#Option_" + category_id + "_Option_" + setting_id);
	} else {
...
const sendToAll = (path_str, val) => {
	GameEvents.SendCustomGameEventToAllClients( "setting_changed", {setting: path_str , value: val });
	GameEvents.SendCustomGameEventToServer( "setting_changed", {setting: path_str , value: val });
}
...
GameEvents.Subscribe( "setting_changed", updatePanel );